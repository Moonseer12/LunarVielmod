...
public class CleanestCleaverProg : ModProjectile
{
    ...
    public override bool OnTileCollide(Vector2 oldVelocity)
    {
        if (Bloody == 3)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, ModContent.DustType<TSmokeDust>(), (Vector2.One * Main.rand.Next(1, 5)).RotatedByRandom(19.0), 180, Color.DarkGray, Main.rand.NextFloat(1, 1.1f)).noGravity = true;
            }
            for (int i = 0; i < 7; i++)
            {
              //  Dust.NewDustPerfect(Projectile.Center, ModContent.DustType<BloodDust>(), (Vector2.One * Main.rand.Next(1, 5)).RotatedByRandom(19.0), 0, Color.OrangeRed, 1f).noGravity = true;
            }
            for (float i = 0; i < 4; i++)
            {
                float progress = i / 4f;
