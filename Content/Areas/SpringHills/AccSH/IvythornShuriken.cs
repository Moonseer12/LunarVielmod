...
public class IvythornShurikenProj : ModProjectile
{
    ...
    public override void OnKill(int timeLeft)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            Dust.NewDustPerfect(Projectile.Center, DustID.JunglePlants, (Vector2.One).RotatedByRandom(25.0), 0, default, 1f).noGravity = false;
        }
        for (int i = 0; i < 5; i++)
        {
            int num1 = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.JunglePlants, 0f, -2f, 0, default, .8f);
            Main.dust[num1].noGravity = true;
            Main.dust[num1].position.X += Main.rand.Next(-50, 51) * .05f - 1.5f;
            Main.dust[num1].position.Y += Main.rand.Next(-50, 51) * .05f - 1.5f;
            if (Main.dust[num1].position != Projectile.Center)
                Main.dust[num1].velocity = Projectile.DirectionTo(Main.dust[num1].position) * 6f;
            int num = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.Dirt, 0f, -2f, 0, default, .8f);
