...
namespace Stellamod.Common.MagicCauldron
{
    ...
    public static class BrewExtension
    {
        ...
        public static CauldronBrew RegisterVanillaBrew(int result, int material, float weight = 1.0f, int yield = 1)
        {
            Cauldron cauldron = ModContent.GetInstance<Cauldron>();
            Cauldron.IsBrewingMaterial[material] = true;
            Cauldron.IsBrewingMold[ModContent.ItemType<BlankAccessory>()] = true;
            Cauldron.MaterialRarity[result] = ItemLoader.GetItem(material).Item.rare;
            return cauldron.AddBrew(result, ModContent.ItemType<BlankAccessory>(), material, 10, weight, yield);
        }
    }
